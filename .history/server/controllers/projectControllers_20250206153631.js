...
export const weeklyProject = async (req, res) => {
    ...
    try {
        ...
        // Assign each reference to the corresponding field in the project schema
        const validatedReferences = {
            reference1: references[0] || '',
            reference2: references[1] || '',
            reference3: references[2] || '',
            reference4: references[3] || ''
        };

        // Create a new project with the validated references
        const newProject = new weeklyProjects({
            title,
            trainer,
            description,
            ...validatedReferences,  // Spread the references into the project object
            doc,
            date
        });
