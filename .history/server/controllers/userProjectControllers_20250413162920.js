...
export const takeProject = async (req, res) => {
    try {
        const tasks = await Userproject.find({}).sort({ createdAt: -1 })
        if (!tasks) {
...
export const getUserProject = async (req, res) => {
    try {
        const userProjects = await Userproject.find({ userId :req.user.id }).sort({ submittedAt: -1 }).populate("User", "firstName email");
