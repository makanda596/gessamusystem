...
eimport { weeklyProjects } from './path-to-your-model'; // Ensure you have the correct path
...
export const weeklyProject = async (req, res) => {
    ...
    try {
        ...
        if (!title || !trainer || !description || !reference || !date) {
        ...
        const midProjects = new weeklyProjects({
            ...
            trainer,
            description,
            reference: Array.isArray(reference) ? reference : [reference], // Ensure it's an array
            doc,
            date
        });
...
export const getweekly-projects = async (req, res) => {
    try {
