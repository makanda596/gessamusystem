...
export const getUserSubmittedTask = async (req, res) => {
    try {
        const submittedTasks = await SubmitTask.findOne({ userId :req.user.id}).sort({ submittedAt: -1 });
...
export const toggleonetask = async (req,res)=>{
    const {id} = req.params
    try {
        const statusComplete = await SubmitTask.findOne({
            _id: id,
            status:"pending"
        })
        if(statusComplete){
            return res.json({message:"the task already being assest"})
        }
        const updatetask = new SubmitTask({
            _id:id,
            status:"completed"
        })
