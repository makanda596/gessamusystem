...
export const useAuthStore = create((set) => ({ //a one argument and a callback function 
    ...
    signup: async (firstName, lastName, admNo, year, email, password, avatar) => {
        ...
        try {
            ...
            const response = await axios.post("http://localhost:5000/auth/signup", { firstName, lastName, admNo, year, email, password, avatar }); //we sending this to the server
            set({ user: response.data, isLoading: false })//updatyes the state of the useAuthstore 
        } catch (error) {
    ...
    login: async (admNo, password) => {
        ...
        try {
            ...
            set({
                user: response.data.user, isAuthenticated: true, error: null, isLoading: false
            })
        } catch (error) {
            set({ error: error.response.data.message || "Error loging up", isLoading: false });  // Error: show error message
            throw error;
    ...
    postProject: async (title, description, year, reference) => {
        try {
            const response = await axios.post("http://localhost:5000/projects/sendprojects", { title, description, year, reference })
            set({ project: response.project })
        } catch (error) {
