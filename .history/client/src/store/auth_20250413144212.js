...
export const useAuthStore = create((set) => ({
    ...
    countTask: async () => {
        try {
            ...
            const response = await axios.get("http://localhost:5000/task/countUserTask", {
                headers: {
    ...
    countAlerts: async () => {
        try {
            const token = localStorage.getItem("token")
            const response = await axios.get("http://localhost:5000/auth/countAlerts", {
                headers: { Authorization: `Bearer${token}` }
            })
    ...
    countUserProjects: async () => {
        try {
            ...
            const response = await axios.get("http://localhost:5000/userprojects/countUserProject", {
                headers: {
    ...
    countCompletedTask: async () => {
        try {
            ...
            const response = await axios.get("http://localhost:5000/task/countUserCompletedTask", {
                headers: {
                    Authorization: `Bearer ${token}` // Added a space after "Bearer"
                }
            });
    ...
    postProject: async (title, description, year, reference) => {
     ...
     fetchTasks : async () => {
        ...
        getUserProjetcs: async ()=>{
            try {
                const token = localStorage.getItem("token")
                const response = await axios.get('http://localhost:5000/userProjects/getUserProject', {
                    headers:{Authorization:`Bearer ${token}`}
                })
    ...
    deleteProject: async(postId)=>{
        try {
            const token = localStorage.getItem("token")
            const response = await axios.delete(`http://localhost:5000/userProjects/deleteUserProject/${postId}`,{
                headers:{Authorization :`Bearer ${token}`}
            })
