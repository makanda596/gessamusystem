...
export const useAuthStore = create((set) => ({ //a one argument and a callback function 
    ...
    login: async (admNo, password) => {
        set({ isLoading: true, error: null });
        try {
            const response = await axios.post("https://lln6pfd1-5000.uks1.devtunnels.ms/auth/login", { admNo, password });
            ...
            // Set state values separately to ensure all updates are processed
            set(state => ({
                user: userData,
