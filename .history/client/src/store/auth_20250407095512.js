...
export const useAuthStore = create((set) => ({
    ...
    login: async (email, password) => {
        set({ isLoading: true, error: null });
        try {
            console.log(import.meta.env); // logs all environment variables
            console.log(import.meta.env.BACKEND_URL); // should print "http://localhost:5000"
    ...
    logout: async () => {
        try { 
            await axios.post(`${process.env.REACT_APP_BACKEND_URL}/auth/logout`, null, { withCredentials: true });
