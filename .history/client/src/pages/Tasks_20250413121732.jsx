...
const Tasks = () => {
    ...
    const { alltasks, fetchTasks }=useAuthStore()
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-gray-100 p-4 md:p-4">
                ...
                <button onClick={() => setIsPopup(true)}>submit</button>
                <SubmitTask isPopup={isPopup} setIsPopup={setIsPopup}/>
                <SubmitProject isPopup={isPopup} setIsPopup={setIsPopup}/>
                {/* Notification Popup */}
                {notification && (
                    <div className="fixed z-10 top-20 left-1/2 transform -translate-x-1/2 bg-blue-500 text-white py-2 px-4 rounded shadow-lg">
                        {notification}
                    </div>
                )}
                ...
                {filteredTasks.length === 0 && !loading && !error ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {displayedTasks.map((task) => (
                            <TaskCard key={alltasks._id} alltasks={alltasks} showNotification={showNotification} />
                        ))}
                    </div>
                )}
            </div>
...
const TaskCard = ({ alltasks, showNotification }) => {
    const [menuOpen, setMenuOpen] = useState(false);
    const [showModal, setShowModal] = useState(false);
    const [title, setTitle] = useState('');
 
    const shareTask = () => {
        const taskUrl = `${window.location.origin}/task/${alltasks._id}`;
        if (navigator.share) {
            navigator
                .share({
                    title: alltasks.title,
                    text: alltasks.description,
                    url: taskUrl,
                })
    ...
    const handleSubmitTask = async () => {
        ...
        try {
            await axios.post(`https://gessamusystem.onrender.com/task/submitTask/${alltasks._id}`, { title });
            showNotification('Task submitted successfully!'); // Show success notification
            setShowModal(false);
            setTitle('');
        } catch (error) {
            showNotification('Failed to submit the task. Please try again.'); // Show error notification
        }
    };
    ...
    return (
        <div>
            <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 hover:shadow-lg transition relative">
                <h2 className="text-xl font-bold text-blue-700 mb-2">{alltasks.task}</h2>
                <p className="text-gray-700 mb-4">{alltasks.description}</p>
                <p className="text-sm text-gray-500 mb-2">
                    Level: <span className="font-medium">{alltasks.level}</span>
                </p>
            ...
            {showModal && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white p-8 rounded-md shadow-lg max-w-md w-full">
                        ...
                        <p className="mb-2"><strong>Selected Task:</strong> {alltasks.task}</p>
