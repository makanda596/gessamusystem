...
const Allprojects = () => {
    ...
    const handleSearch = debounce(async () => {
        ...
        try {
            ...
            if (filteredProjects.length > 0) {
                setSearchResults(filteredProjects); // Store filtered results in searchResults
            } else {
    ...
    const currentProjects = searchTerm ? searchResults.slice(indexOfFirstProject, indexOfLastProject) : projects.slice(indexOfFirstProject, indexOfLastProject); // Display search results or all projects
    const totalPages = Math.ceil((searchTerm ? searchResults.length : projects.length) / projectsPerPage);
                    ...
                    <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        {currentProjects.length > 0 ? (
                            currentProjects.map((project, index) => (
                                ...
                                >
                                    <h2 className="text-lg font-semibold text-black p-1 rounded-md transition-colors duration-300">
                                    ...
                                    {project.reference1 && (
                                        <p className="text-md text-black font-bold">
                                            Link 1:{" "}
                                            <a href={project.reference1} target="_blank" rel="noreferrer" className="text-blue-600 font-sans underline hover:text-green-600">
                                                {project.reference1}
                                            </a>
                                        </p>
                                    )}
                                    {project.reference2 && (
                                        <p className="text-md text-black font-bold">
                                            Link 2:{" "}
                                            <a href={project.reference2} target="_blank" rel="noreferrer" className="text-blue-600 font-sans underline hover:text-green-600">
                                                {project.reference2}
                                            </a>
                                        </p>
                                    )}
                                    {project.reference3 && (
                                        <p className="text-md text-black font-bold">
                                            Link 3:{" "}
                                            <a href={project.reference3} target="_blank" rel="noreferrer" className="text-blue-600 font-sans underline hover:text-green-600">
                                                {project.reference3}
                                            </a>
                                        </p>
                                    )}
