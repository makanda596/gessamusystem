                        ...
                        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {currentProjects.length > 0 ? (
                                currentProjects.map((project, index) => (
                                    <div key={index} className="mb-6 p-6 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:scale-105">
                                        <h2 className="text-3xl font-bold text-white bg-blue-500 py-2 px-4 rounded-lg shadow-md hover:bg-blue-600 transition-all duration-300">
                                            <a href={`/project/${project._id}`} target="_blank" rel="noreferrer">{project.title}</a>
                                        </h2>
                                        <p className="text-gray-200 mt-2">{project.description}</p>

                                        <div className="my-4">
                                            <p className="text-lg text-gray-100 font-semibold">{project.doc ? 'Document Attached' : 'No Document Available'}</p>
                                            <img
                                                src={project.doc || "/images/default.jpg"}
                                                alt={project.task || "Task Image"}
                                                className="mt-2 w-full h-56 object-cover rounded-md shadow-md"
                                            />
                                        </div>

                                        <div className="mt-4">
                                            <p className="text-sm text-gray-200">Link:
                                                <a className="text-blue-400 hover:underline" href={project.reference} target="_blank" rel="noreferrer">{project.reference}</a>
                                            </p>
                                            <p className="text-sm text-gray-200">Trainer: <span className="font-semibold text-yellow-300">{project.trainer}</span></p>
                                            <p className="text-sm text-gray-200">Date: <span className="font-semibold text-green-300">{project.date}</span></p>
                                        </div>
                                    </div>
                                ))
                            ) : (
